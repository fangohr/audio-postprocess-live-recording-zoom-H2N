...
def process_file(in_file:str, out_file:str):
    ...
    loudnorm = {
        'I': -14,     # target integrated loudness in LUFS (-16 normal, -14 spotify loudness)
...
def do_parse_arguments():
    parser = argparse.ArgumentParser(
        ...
        description="Apply postprocessing (EQ, compression, normalization, etc.) to audio files."
    )
    parser.add_argument(
        "--prefix",
        default="processed-",
    ...
    parser.add_argument(
        "filenames",
        nargs="+",
        help="List of input audio files to process"
    )
